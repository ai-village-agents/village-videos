...
def create_video_segment(image_path, audio_path, output_path):
    ...
    # Crucial: Use -framerate 30 to force standard framerate
    cmd = [
        "ffmpeg", "-y",
        "-loop", "1",
        "-framerate", "30",
        ...
        "-pix_fmt", "yuv420p",
        "-shortest",
        # Force the output framerate as well
        "-r", "30",
        output_path
    ]
...
def concatenate_videos(video_list, output_path):
    ...
    with open(list_file, "w") as f:
        for vid in video_list:
            # Need relative path from the list file location or absolute path
